...
{	
    ...
    spritecolor.rgb = blendMultiply(spritecolor.rgb, satinColor.rgb, satinColor.a)*spritecolor.a; //apply satin (but no blur)
    ...
    {
        vec4 col = flixel_texture2D(bitmap, uv + vec2(offsetX*(distMult.x*i), offsetY*(distMult.y*i)));
    ...
    spritecolor.rgb = blendLighten(spritecolor.rgb, overlayColor.rgb, overlayColor.a)*spritecolor.a; //apply overlay
